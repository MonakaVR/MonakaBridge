...
int main()try{
 ...
 next.sequence=2;b.receive(wire(next),"p",15000000);CHECK(b.logical().at({"pico","device"}).pose);
 mb::MtpFeed feed;auto first=*b.logical().at({"pico","device"}).pose;first.sent_at_ns=16000000;
 CHECK(feed.receive(wire(first),10000000));auto key=mb::Key{c.bridgeId,"logical-pico"};CHECK(feed.devices.at(key).fixedTime==8000000);
 ...
 CHECK(feed.receive(wire(restarted),20000000));CHECK(!feed.receive(wire(first),21000000));
 auto cfg=c;cfg.revision=2;cfg.bindings.at({"other","device"}).tracker="logical-pico";cfg.bindings.at({"pico","device"}).tracker="new";
 bool failed=false;try{b.reconfigure(cfg,16000000);}catch(...){failed=true;}CHECK(failed);CHECK(b.config().revision==1);
 ...
 cfg=c;cfg.revision=2;b.reconfigure(cfg,17000000);CHECK(!b.logical().at({"pico","device"}).pose->angular_velocity);
