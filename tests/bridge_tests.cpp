...
int main()try{
 ...
 p.session_id=p.clock_id=S2;p.sequence=0;CHECK(r.receive(wire(p),"a",410000000));CHECK(!r.receive(wire(sample()),"a",420000000));CHECK(r.find({"other","device"}));
 CHECK(!r.receive(wire(p),"imposter",420000000));CHECK(r.collisions==1&&!r.find({"pico","device"}));
 ...
 auto restart=sample();restart.session_id=restart.clock_id=S2;bridge.receive(wire(restart),"a",13000000);CHECK(bridge.logical().at({"other","device"}).pose);CHECK(!bridge.logical().at({"pico","device"}).pose->angular_velocity);
 bridge.tick(600000000);CHECK(!bridge.logical().at({"pico","device"}).pose);
 ...
 std::ifstream vive(std::string(ROOT_DIR)+"/tests/fixtures/vive-status-samples.jsonl");std::string line;int count=0;
 while(std::getline(vive,line)){mb::Registry vr;CHECK(vr.receive(line,"vive",100000000));++count;}CHECK(count==7);
 std::cout<<"PASS registry/calibration/mapping/routing/config/UDP and 7 verified VIVE C1 fixtures\n";return 0;
