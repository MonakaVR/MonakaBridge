...
def run(name,cmd,timeout=90):
 ...
 text=r.stdout.decode('utf-8',errors='replace')+r.stderr.decode('utf-8',errors='replace');(evidence/(name+'.log')).write_text(text,encoding='utf-8');results[name]={'exit_code':r.returncode,'result':'PASS' if r.returncode==0 else 'FAIL'};checkpoint('RUNNING' if r.returncode==0 else 'FAIL');print(name,results[name],flush=True);assert r.returncode==0,text;return text
