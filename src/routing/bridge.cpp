...
template<class T>void header(T& out,const Config& c,const std::string& session,const Binding& b,std::int64_t seq,std::int64_t timestamp,std::int64_t now){out.version={1,0};out.source_id=c.bridgeId;out.session_id=out.clock_id=session;out.tracker_id=b.tracker;out.sequence=seq;out.timestamp_ns=timestamp;out.sent_at_ns=now;out.timestamp_kind="receive";out.coordinate_space={b.worldSpace,"rh_y_up_neg_z_forward",b.worldRevision};out.mapping_revision=c.revision;}
}
void Fanout::put(Channel c,c1::Envelope e,std::int64_t now){auto& q=queues_[c];
 if(auto state=std::get_if<c1::MtpTrackerState>(&e);state && (state->presence=="absent"||state->tracking_state=="disconnected")){
...
Bridge::Bridge(Config c,std::string session):config_(std::move(c)),session_(std::move(session)){validateConfig(config_);registry.timeoutNs=config_.timeoutNs;for(auto& [key,b]:config_.bindings)trackerOwners_.emplace(b.tracker,key);}
...
void Bridge::tick(std::int64_t now){
 for(const auto& [key,b]:config_.bindings){
  ...
  if(fresh && d->pose && (l.inputSequence!=d->poseSequence || !l.pose)){
   ...
   out.input={in.device_id,in.session_id,in.sequence};l.inputSequence=d->poseSequence;l.pose=out;publish(out,now);
  }
...
void Bridge::reconfigure(Config c,std::int64_t now){
 ...
 for(auto& [key,b]:c.bindings){auto old=owners.find(b.tracker);if(old!=owners.end()&&old->second!=key)throw std::invalid_argument("tracker reassignment requires Bridge restart");owners.emplace(b.tracker,key);lifetimeKeys.insert(key);}
 ...
 for(auto& [key,l]:logical_){const auto old=config_.bindings.find(key);if(old!=config_.bindings.end()){c1::MtpTrackerState s;header(s,config_,session_,old->second,l.stateSequence++,now,now);s.presence="absent";s.tracking_state="disconnected";publish(s,now);}l.history=History{};l.pose.reset();l.stateSignature.clear();}
